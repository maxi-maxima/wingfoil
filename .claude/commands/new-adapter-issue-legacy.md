...
This command is the scoping counterpart to `/new-adapter-legacy`. The issue it files should read as a faithful pre-implementation plan: anyone picking it up later (or running `/new-adapter-legacy $ARGUMENTS`) should find every decision already framed.
...
0. **Provenance** — the very first line of the issue body must state, verbatim:
   ...
   > _This issue was generated by the `/new-adapter-issue-legacy` skill. Any implementation picking it up **must** be validated against the `/new-adapter-legacy` skill — every step, file layout, feature flag, test harness, and deliverable in that skill is authoritative, and deviations must be justified in-thread before merging._

   Do not paraphrase or omit this line; it is what signals to future implementers (human or agent) that `/new-adapter-legacy` is the normative spec for the work.
...
4. **File layout** — the directory tree under `legacy/wingfoil/src/adapters/$ARGUMENTS/` (`mod.rs`, `read.rs`, `write.rs`, `integration_tests.rs`, `CLAUDE.md`), called out against the `new-adapter-legacy` step-5 defaults and any deviation justified.
...
10. **Deliverables / acceptance criteria** — checklist mirroring the `/new-adapter-legacy` steps. The final box is mandatory and enforces the validation contract from the provenance line:
    ...
    - [ ] Implementation validated end-to-end against the `/new-adapter-legacy` skill (every step followed or deviation justified in-thread)
...
- Do not check out branches, edit code, or run `/new-adapter-legacy` as a follow-up unless explicitly asked.
- If the user then asks to implement, hand off to `/new-adapter-legacy $ARGUMENTS`.
