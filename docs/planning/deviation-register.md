...
block and not themselves sources. (2) `docs/planning/port-plan.md` (capability matrix +
...
| A5 | ~~**Caller-owned tokio runtime**~~ — etcd/redis/kafka/postgres/otlp took `&Handle`. | ✅ | **Resolved (#548).** The `GraphBuilder` now owns one tokio runtime (lazy, shared, dropped at teardown) with a `with_async_runtime` override; the `&Handle` param is gone from every async factory. Decision record: [`runtime-ownership.md`](../decisions/runtime-ownership.md). *Residual (A5a below) is unchanged.* |
...
| C4 | **Compiled-path IO ingestion** — ~~busy-poll (`ALWAYS`) sources~~ + bursts are not expressible in `compiled()`/`nitro!`. | ⚪ | **The busy-poll half has since landed**, and it landed *alone* — which this row's original "these land together or the exclusion stays" coupling did not expect. `poll` now reaches both compiled tiers: `expand_compiled` derives "this graph spins" from the OR of its ops' `ACTIVATION` consts and calls `Kernel::set_spin`, and an island declares `always` outward so the *outer* engine spins. Pinned by `tests/poll_all_tiers.rs`; capability matrix row "Busy-poll ingest (`ALWAYS`)" is ✅ across all four columns, footnote ¹⁷. **What remains deferred is wake-driven ingest**: `external`/`channel` (`THREADED`) sources cannot set a dirty bit in a `Kernel` built without a ready receiver, and bursts go with them — no burst *source* exists in the macro vocabulary, and the pattern is about the IO edge the compiled path still excludes. So compiled realtime is timer- or poll-driven, not wake-driven; wake-driven IO stays at the interpreted boundary with compiled islands inside. The open gating decision (wake channel vs busy-spin) is unchanged and in `port-plan.md` "Deferred / post-v1 work"; the busy-spin answer is the one the landed half took. (was #502/#503) **Cutover ruling 2026-08-03: stays post-v1** (cutover-plan 2.8) — a deliberate exclusion, and not a parity gap since legacy has no compiled tier. |
...
| C6 | ~~**`Graph::export` (GML topology dump) not ported.**~~ | ✅ | **Resolved — superseded, not restored.** The drop was ruled correct at cutover (2026-08-03) on the grounds that a *designed* introspection surface should replace it rather than a same-shape port; that surface has now landed as `crates/wingfoil/src/introspect.rs`. `GraphBuilder::snapshot()` / `Runner::snapshot()` return a `GraphSnapshot` — a **value**, assertable in a test, rather than a side effect on a file path — carrying every node's label and `Activation` and, unlike GML, keeping **active and passive edges distinct**. It renders to text, Mermaid (GitHub draws it inline), Graphviz DOT, JSON and `to_gml()`, the last byte-compatible in structure with legacy's output so existing yEd/Gephi tooling still works. So the legacy capability is a strict subset of what ships. Tests: `tests/introspect.rs` (10 cases, including a serde round-trip pinning the hand-written JSON against the derives, and a `dynamic-graph` case pinning that `removed` stays index-aligned across a runtime splice and removal) plus 9 unit tests over the renderers. `examples/core/introspect/`; design and the remaining steps in `docs/planning/introspection-plan.md`. |
| C7 | ~~**Latency ops are fluent/interpreted-only** — `stamp` / `stamp_precise` / `latency_report` have no `nitro!` / `compiled()` / `nested()` form.~~ | ✅ | **Resolved for the stamps** (cutover ruling 2026-08-03, cutover-plan row 2.2); `latency_report` reclassified as structural, not a gap. `stamp` / `stamp_precise` now work in all three expansions. The blocker was real but the stated fix — "per-op type-argument syntax in `nitro!`" — was the wrong shape: Rust wants *all* of a function's type arguments or none, and the macro never learns a forwarder's arity (it only ever sees a method-name token; never naming the op type is the design). So the stage crosses as a **value whose type carries it** — `#[op(explicit = S)]` gives each forwarder a leading `PhantomData<S>` and the emission passes `PhantomData::<the_stage>`, so inference resolves it from an argument like any other parameter. The mechanism is general: any op with a phantom type parameter can now reach the compiled tiers. **`latency_report` stays interpreted-only by structure, not by omission**: the sink's entire value is the `LatencyHandle` it returns, and `compiled()` is outputs-only by design, so the handle cannot escape — a compiled `latency_report` could print at teardown but never be read. The same reasoning now also covers `stamp_as` / `stamp_all`, which choose *which* node to insert (or whether to insert one) from a runtime value: that is a wiring-time branch, and a `nitro!` graph's topology is fixed at expansion. The four named stamps remain dual-mode, which is what the entry was about. Tests: `stamps_reach_the_compiled_tier` and `stamps_reach_a_nested_island` in `tests/latency.rs`. `src/latency.rs`; `port-plan.md` Phase 5 "Latency". |
...
| D13 | **The Python latency surface returns its stats handle, and the `_if` variants differ from legacy's.** `wingfoil.latency_report(stream, stages)` returns `(sink, LatencyStats)` where legacy `wingfoil` returned only the sink node; `latency_report_if(..., enabled=False)` returns a sink that never ticks (plus an empty stats handle) where legacy returned the *upstream* node. | 🟢 | Both follow **wingfoil's own engine**, not legacy Python: `LatencyReportOps::latency_report` already hands the shared `LatencyStats` back so a caller can read the numbers after the run, and `latency_report_if` already returns a never-ticking sink when disabled. The Python surface being the odd one out was the deviation; this removes it. The disabled variant also keeps the call's return *shape* constant, which legacy's upstream-passthrough did not. Cross-cutting because it is the first binding whose signature is widened by an engine capability legacy never had. `wingfoil-python/src/latency.rs` deviations 2 and 7. Widened again by the latency-API work: the Python surface also carries `stamp_as` / `stamp_all` (the engine's `Stamping` mode and fused stage set, the latter worth more here than in Rust — it collapses N GIL attaches to one), `LatencyStats.total` / `.hops()` / `.reset()`, and `output=` in place of `print_on_teardown=` so the summary can go to the log rather than only stdout. Legacy has none of it; deviation 8. |
...
| D21 | **web: `Complete` is emitted from the sink's teardown**, not from the consumer noticing its source ended. | 🟢 | `consume_async_bursts` hands back a `flush` teardown (the **B1** mechanism); `web_pub` chains its own `finally` that flushes every queued frame, joins the consumer, and *then* sends `Complete { topic }`. The marker still arrives strictly after the last data frame, on the topic's own channel — and since **D30** that means whichever of the two delivery paths the run resolved to, the lossless one included — for both a finite `RunFor` and the end of a historical replay, so the observable protocol is unchanged. `adapters/web/mod.rs` deviation 4. |
...
| D28 | **The `_if` stamps are withdrawn: `stamp_if` / `stamp_precise_if` (and the burst `stamp_each_if` / `stamp_precise_each_if`) no longer exist.** Legacy offers `stamp_if` / `stamp_precise_if` on `LatencyStreamOps`, and wingfoil carried both until the latency-capture API rework. `Stamping` replaces them: `stamp_if(on)` is `stamp_as(Stamping::on_if(on))`, `stamp_precise_if(on)` is `stamp_as(Stamping::new(on, true))`. | 🟡 | **The one place the stamping surface is deliberately not a superset of legacy's.** The capability is untouched — `Stamping::Off` wires no node, exactly as `_if(false)` did, and every call the `_if` forms served is one `stamp_as`; what is withdrawn is the *spelling*, and only because keeping it is actively harmful. The `_if` pair cannot express "precise or not, decided at runtime" — the shape a `--precise-stamps` flag has — so that case had to be written as two nodes with opposite polarities (`stamp_if(!p).stamp_precise_if(p)`), where dropping one `!` stamps the stage twice, the second write silently overwriting the first with no error and a plausible-looking report. `trading_e2e` carried eighteen such calls before the rework. A method whose only remaining use is a footgun is not surface worth preserving for parity's sake, and the deviation is recorded here rather than the methods kept. The two burst forms need no parity argument at all: legacy has no burst-shaped stamp, so they were wingfoil-only surface and are simply deleted. **Python keeps its `stamp_if` / `stamp_precise_if`** (`wingfoil-python/src/latency.rs`) — they are legacy Python entry points (port-plan Phase-5 latency), free functions rather than trait methods so they crowd nothing, and `examples/latency.py` uses them; the trait-surface argument does not transfer. Legacy-recorded behaviour survives as `tests/latency.rs::stamp_disabled_is_identity`, re-pinned to `Stamping::on_if(false)` — the literal translation — so the recorded behaviour outlives the surface that expressed it. `latency_report_if` is **not** part of this: `ReportOutput::Silent` still wires the sink and accumulates, so no `Stamping`-shaped argument expresses "aggregate nothing", and that toggle stays a method. `src/latency.rs`; migration guide "Latency capture". |
| D29 | **The `Signal` facade is withdrawn: `wingfoil::signal` — free `ticker`/`constant` source functions plus `Signal<T>` with `run`/`peek_value` — no longer exists.** It was the last spelling of legacy's builder-less idiom (`nodes::ticker(period)` … `stream.run(..)` / `stream.peek_value()`); every wingfoil program now takes the three explicit steps: wire streams from a `GraphBuilder`, `build()` the graph, `run(..)` the `Runner`. | 🟡 | **A spelling withdrawn, not a capability** — everything the facade expressed is one explicit `GraphBuilder` away, and it was withdrawn because its shape is a trap, the same reasoning as D28. Each free source function minted its own implicit graph, so the first two-source program a user writes in the facade — `ticker(a).merge(&ticker(b))` — panics at wiring time ("Handle used with a different Builder than the one that minted it"): every combining op (`merge`/`join`/`sample`/…) only ever worked on signals descended from a single source call. That is structural — the implicit per-source graph *is* the facade — and the fixes all cost more than the surface was worth: a thread-local ambient builder either makes `run` execute every node wired on the thread (side effects included) or requires reachability-scoped subgraph builds the engine does not have. Cutover item 1.4 had already ruled there is no legacy-compat facade; this row reverses the module's separate "stands on its own" justification. The engine contract the facade exercised — build once, re-run restores wiring-time state — was already pinned at the `Runner` level (`tests/rerun.rs`, `tests/op_builder_shapes.rs`, `tests/fluent_primitives.rs`), so no recorded behaviour is lost with `tests/signal.rs`. `#[op(fluent)]` no longer emits the `__wf_signal_<name>!` twin; `/new-op` step 4b is gone with it. Python is untouched — `wingfoil-python` never wrapped `Signal`. |
| D30 | **web: a historical replay paces itself to its subscribers; legacy's transport is lossy in both run modes.** `WebServerBuilder::delivery` takes a `Delivery { Auto, Lossy, Lossless }`, defaulting to `Auto` — lossy under `RunMode::RealTime`, lossless under `RunMode::HistoricalFrom`. Legacy has one behaviour: an unbounded queue drained into a 1024-slot `broadcast` that cannot block its sender, plus a `try_send` that drops on a full outbound queue. | 🟡 | **The one place wingfoil's web adapter deliberately behaves differently from legacy on the same graph**, and the reason it is 🟡 rather than 🟢: a historical run that legacy would race through, dropping frames, now waits for the socket. That is the fix for [#437](https://github.com/wingfoil-io/wingfoil/issues/437) — legacy's rule ("a frozen browser tab can never back-pressure the graph") is right in real time, where a client that falls behind is already showing stale data, and wrong in a replay, which has no live clock to fall behind: a backtest at CPU speed outruns any socket, so dropping does not keep up with anything, it just corrupts what the browser draws (a probe measured 4586 of 5000 frames delivered to one loopback subscriber). **Real time keeps legacy's contract — nothing a client does can stall the graph, and frames drop on a full outbound queue — but not its plumbing**: the rework replaced the unbounded sink channel draining into a 1024-slot `broadcast` with a single **bounded** sink channel (`PUBLISH_INFLIGHT_INSTANTS` = 64 instants in `write.rs`) whose consumer writes straight into each connection's 1024-slot outbound queue. Two measurable consequences: a graph that outruns its own envelope encoding for 64 consecutive instants now waits on the sink channel rather than growing memory without bound, and a briefly-stalled live client starts dropping after roughly one queue's worth of buffered frames (~1024) instead of two (~2048, queue plus broadcast). `Delivery::Lossy` restores the never-block, drop-on-full behaviour in both modes for a caller who wants it. Zero subscribers never waits (an unwatched replay runs at full speed), and several subscribers pace to the slowest — bounded by `WebServerBuilder::lossless_stall_timeout` (default 30 s) so "slowest" cannot mean a half-open peer that never closes its socket, which would otherwise park the graph until TCP keepalive and hang `run()` itself on the end-of-run `Complete`; a subscriber withdrawn that way has its connection closed, so a client that recovers reconnects instead of holding a socket that will never carry another frame. The resolved policy is stored **per publish**, not per server, so two graphs sharing one `WebServer` in opposite run modes each get their own answer. `adapters/web/{server,write}.rs`; `adapters/web/mod.rs` deviation 6; `tests/web_integration.rs::historical_streaming_is_lossless_for_a_fast_graph`. |
