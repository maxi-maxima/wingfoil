...
| Trivial state/closure | ✅ distinct, drop_small_change, difference, pairwise, limit, skip, take_while, map_filter, throttle, inspect, logged, window, buffer, with_time, ticked_at/-elapsed, not, print, timed (`tests/catalog.rs`, `tests/catalog_ops.rs`, `tests/catalog_flow.rs`); ✅ split, combine, collapse (Burst/tuple structural, `tests/catalog_flow.rs`) | recipe proven; `skip` and `take_while` are new surface with no legacy twin and have three-engine parity in `tests/compiled_stateful_ops.rs`; `take_while` latches quiet at the first rejected value rather than terminating the run. `pairwise` likewise is new surface — it is `difference` without the `Sub` bound, tupling where `difference` subtracts, so it reaches types with no arithmetic (`tests/catalog.rs`, three-engine parity in `tests/compiled_stateful_ops.rs`); `window`/`buffer` use `Ctx::is_last_cycle`; `combine` builds the burst locally (no shared-cell port). `print` prints per-tick (deviation D8, dropping legacy's teardown buffer); `logged` is fluent-only (deviation D9) — `&str` label vs `String` cfg. `not` and `collapse` were fluent-only sugar (over `map` / `map_filter`) until promoted to real ops (`ops::Not` / `ops::Collapse`, `tests/catalog_not_collapse.rs`); both are dual-mode now, exercised in `op_completeness.rs::surface_sugar`. `split` stays fluent-only and cannot be promoted — two output streams against an `Op`'s single `Out` |
