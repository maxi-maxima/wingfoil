...
| Trivial state/closure | ✅ distinct, drop_small_change, difference, pairwise, enumerate, limit, skip, skip_while, step_by, take_while, map_filter, throttle, start_with, audit, inspect, logged, window, buffer, with_time, ticked_at/-elapsed, not, print, timed (`tests/catalog.rs`, `tests/catalog_ops.rs`, `tests/catalog_flow.rs`); ✅ split, combine, collapse (Burst/tuple structural, `tests/catalog_flow.rs`) | recipe proven; `skip`, `skip_while`, `step_by`, `take_while`, `start_with`, and `audit` are new surface with no legacy twins; `start_with` emits a real tick at the declared run start, with the source winning a same-time tie, and has exact value/time plus three-engine coverage. `audit` is the fixed-window trailing-edge sibling of `throttle`, with exact value/time, continuously-busy, zero-window, and final-flush coverage in `tests/catalog_flow.rs` plus interpreted/compiled parity in `tests/op_completeness.rs`. `skip_while` latches *open* at the first rejected value — the value that opens the latch is emitted and the predicate is never called again, pinned by a predicate-call count across all three engines — and carries a hand-written Python binding (`crates/wingfoil-python/src/graph.rs`, `tests/test_interop.py`) because a Python predicate can raise where the op's `Fn` cannot; `step_by` is the count-domain twin of `throttle` and returns a run error for a zero step instead of panicking; `take_while` latches quiet at the first rejected value rather than terminating the run. `pairwise` likewise is new surface — it is `difference` without the `Sub` bound, tupling where `difference` subtracts, so it reaches types with no arithmetic (`tests/catalog.rs`, three-engine parity in `tests/compiled_stateful_ops.rs`); `enumerate` adds the zero-based per-stream context no composition of the existing ops recovers without an extra node and a one-based correction (`tests/catalog.rs`, three-engine and rerun parity in `tests/compiled_stateful_ops.rs` / `tests/rerun.rs`). `window`/`buffer` use `Ctx::is_last_cycle`; `combine` builds the burst locally (no shared-cell port). `print` prints per-tick (deviation D8, dropping legacy's teardown buffer); `logged` is fluent-only (deviation D9) — `&str` label vs `String` cfg. `not` and `collapse` were fluent-only sugar (over `map` / `map_filter`) until promoted to real ops (`ops::Not` / `ops::Collapse`, `tests/catalog_not_collapse.rs`); both are dual-mode now, exercised in `op_completeness.rs::surface_sugar`. `split` stays fluent-only and cannot be promoted — two output streams against an `Op`'s single `Out` |
| Predicate / accumulator twins | ✅ filter_value, scan (`tests/catalog_filter_value_scan.rs`) | `filter_value` closed a **legacy parity gap** — `NodeOperators::filter_value` shipped on legacy but wingfoil carried it only on `Signal`, the `dynamic-graph` `Extension` and the Python binding, never on `StreamOps`; the wingfoil op is one node against legacy's `map` + `FilterStream` pair (pinned by a `node_count` assertion). `scan` is new surface with no legacy twin — `fold` with the closure returning the accumulator; both dual-mode via `surface_u64` in `tests/op_completeness.rs`. Neither needs a Python binding: `filter_value` is already bound, and Python's `fold` *is* `scan` (its callable returns the accumulator, there being no `&mut`) |
| Scheduling | ✅ throttle, start_with, audit, delay_with_reset, node_flow (node-level delay/filter/limit/throttle/audit, run over the unit-stream path, `tests/catalog_flow.rs`) | `SCHEDULES`/time-gated; pattern proven by delay + start_with + throttle + audit |
...
- **Examples**: port all (order_book, breadth_first, run_mode, latency,
  ...
  two defects in the legacy pair, see that example's README), `trading_e2e`
  (`examples/showcase/trading_e2e/{ws_server,fix_gw}.rs` plus the whole engine-agnostic
  ...
  `*-trading-e2e-*.yml` workflows still target the legacy copy by path;
...
- **Benchmarks**: the four-way `tiers` bench 🟢 *landed* — each workload now
  ...
  **The legacy bench suite is now ported too** 🟢 *landed* — all eight legacy
  ...
  `bench` gate — see below — leaving `graph` as the only `add_bench` consumer.)
  Workloads are kept identical so a wingfoil
