...
| Scheduling | ✅ throttle, audit, debounce, delay_with_reset, node_flow (node-level delay/filter/limit/throttle/audit/debounce, run over the unit-stream path, `tests/catalog_flow.rs`) | `SCHEDULES`/time-gated; pattern proven by delay + throttle + audit + debounce |
