...
|---|---|---|---|---|---|---|
| [**Wingfoil**](https://github.com/wingfoil-io/wingfoil) | Rust | **Rust** first; Python, TypeScript | ~27 ns/node-cycle. Matched ingest 151–156 ns interpreted, 50–54 ns compiled; +17.5 / +5.5 ns per extra consumer ⬥ | Latency-critical compute graphs — trading, decisioning, streaming features; backtest then live unchanged | Native API, no interpreter in-process; one wiring runs interpreted *or* compiled; per-hop latency tracing | Youngest here; an engine, not a platform — no order/position/venue model; adapter breadth behind csp and Nautilus |
