...
The cross-language tests **fail** rather than skip when `import wingfoil`
...
`wingfoil-python` feature `zmq = ["wingfoil/zmq", "_common"]`.
...
- `zmq_sub` is the binding that made `#[pyadapter]` accept a **tuple return**:
  `data, status = wingfoil.zmq_sub(g, "tcp://localhost:5556")`. `status`
