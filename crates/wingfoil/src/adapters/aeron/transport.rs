...
/// In-memory [`AeronSubscriberBackend`] that replays canned poll batches.
/// Test support — no media driver required.
...
impl MockSubscriber {
    ...
    pub fn from_messages(messages: Vec<Vec<u8>>) -> Self {
...
/// In-memory [`AeronPublisherBackend`] that records every offered buffer.
/// Test support — no media driver required.
#[derive(Default)]
pub struct MockPublisher {
    /// Every buffer offered so far, in order.
    pub published: Vec<Vec<u8>>,
}

impl MockPublisher {
    /// A publisher with an empty log.
    pub fn new() -> Self {
        Self {
            published: Vec::new(),
        }
