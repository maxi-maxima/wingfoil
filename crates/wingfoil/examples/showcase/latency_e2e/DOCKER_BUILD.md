...
```bash
# Wingfoil binaries
docker build -f crates/wingfoil/examples/showcase/latency_e2e/Dockerfile.ws_server -t wingfoil-ws-server:latest .
docker build -f crates/wingfoil/examples/showcase/latency_e2e/Dockerfile.fix_gw    -t wingfoil-fix-gw:latest    .

# Observability — base image + COPY of the matching config dir
docker build -f crates/wingfoil/examples/showcase/latency_e2e/Dockerfile.prometheus -t wingfoil-prometheus:latest .
docker build -f crates/wingfoil/examples/showcase/latency_e2e/Dockerfile.tempo      -t wingfoil-tempo:latest      .
docker build -f crates/wingfoil/examples/showcase/latency_e2e/Dockerfile.grafana    -t wingfoil-grafana:latest    .
...
```bash
cd crates/wingfoil/examples/showcase/latency_e2e
...
```bash
cd crates/wingfoil/examples/showcase/latency_e2e/pulumi/fargate
...
- To speed up: ensure `cargo build` works locally first (`cargo build --manifest-path crates/wingfoil/Cargo.toml --release --example latency_e2e_ws_server`)
