...
```bash
cd crates/wingfoil/examples/showcase/latency_e2e/pulumi/baremetal
...
cargo build --release --manifest-path crates/wingfoil/Cargo.toml --example latency_e2e_ws_server \
    --features "web-tls,iceoryx2,prometheus,otlp"
cargo build --release --manifest-path crates/wingfoil/Cargo.toml --example latency_e2e_fix_gw \
