...
//! [`RunMode::HistoricalFrom`] instead of real time. The publisher then replays
//! a finite series (500 points) with a small per-point delay that stands in for
//! a genuinely slow computation, so the browser can follow the replay live. When
...
fn main() -> anyhow::Result<()> {
    ...
    // Publish a synthetic mid-price stream. In real time this ticks at 100 Hz
    // forever; in historical mode it replays a finite 500-point series with a
    // small per-point delay standing in for a slow computation, so the browser
    // can watch the replay unfold.
    let _price = g
        .ticker(Duration::from_millis(10))
        .count()
        .map(move |n: &u64| {
            if historical {
                // Simulate a slow per-point computation so the replay is paced
                // for a human watching in the browser rather than flushing 500
                // points in microseconds.
