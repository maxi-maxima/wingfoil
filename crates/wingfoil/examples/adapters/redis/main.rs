...
//!
//! A port of the legacy `legacy/wingfoil/examples/redis` example onto the wingfoil engine.
...
use wingfoil::adapters::redis::{RedisConnection, RedisEntry, RedisSinkOps, redis_sub};
...
fn main() -> anyhow::Result<()> {
    ...
    let processor = std::thread::spawn(move || -> anyhow::Result<()> {
        ...
        let _sink = redis_sub(&g, realtime(2).run_mode, processor_conn.clone(), SOURCE)?
            .map(|burst: &Burst<_>| {
                burst
                    .iter()
                    .map(|event: &wingfoil::adapters::redis::RedisEvent| {
                        let upper = event
                            .payload_str()
                            .unwrap_or("")
                            .to_uppercase()
    ...
    let verifier = std::thread::spawn(move || -> anyhow::Result<()> {
        ...
        let _print = redis_sub(&g, realtime(2).run_mode, verifier_conn, DEST)?
            .collapse()
            .for_each(|event: &wingfoil::adapters::redis::RedisEvent| {
                println!(
                    "  {} -> {}",
                    event.channel,
