...
//! a wingfoil graph — the legacy `produce_async` model, ported to next.
//!
...
//!
//! ```sh
//! RUST_LOG=info cargo run --manifest-path crates/wingfoil/Cargo.toml --features async --example async
//! ```

use std::time::Duration;

use wingfoil::async_source::produce_async;
