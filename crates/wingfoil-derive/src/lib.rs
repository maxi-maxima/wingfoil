...
//!        compiled graph: it is sugar over two `map`s — bind them separately:
//!        `let a = pairs.map(|t| t.0.clone()); let b = pairs.map(|t| t.1.clone());`
//! ```
//!
//! This list is short by design. Sugar that *can* become a real op is promoted
//! rather than listed — `not` and `collapse` were both rejected here until they
//! became [`ops::Not`] / [`ops::Collapse`] and started working outright.
//!
...
fn expand_interpreted(def: &NitroDef) -> TokenStream2 {
    ...
    quote! {
        ...
        pub fn interpreted() -> (
            ::wingfoil::interp::Runner,
            #(::wingfoil::interp::Handle<#out_types>,)*
        ) {
...
fn ctx_expr(target: Target, idx: usize) -> TokenStream2 {
    match target {
        Target::Compiled => quote! { ::wingfoil::op::Ctx::new(&mut __k, #idx) },
        Target::Nested => {
            quote! {
                ::wingfoil::op::Ctx::nested(
...
fn expand_compiled(def: &NitroDef) -> TokenStream2 {
    ...
    quote! {
        ...
        pub fn compiled(
            ...
            let mut __first_err: ::core::option::Option<::wingfoil::anyhow::Error> =
                ::core::option::Option::None;
            let __run = (|| -> ::wingfoil::anyhow::Result<()> {
                #(#starts)*
...
fn expand_nested(def: &NitroDef) -> TokenStream2 {
    ...
    quote! {
        ...
        #[allow(unused_mut, unused_variables, unused_assignments)]
        pub fn nested(
            __g: &::wingfoil::fluent::GraphBuilder,
            #(#in_names: &::wingfoil::fluent::Stream<#in_tys>,)*
            ...
            __g.__composite(__active, __passive, #callback_activated, #island_spins, move |__ctx, __phase| {
                ...
                ::core::result::Result::Ok(if #out_t {
                    ::wingfoil::op::Tick::Value(::core::clone::Clone::clone(&#out_v))
                } else {
                    ::wingfoil::op::Tick::Quiet
                })
            })
        }
    }
