//! Python bindings for the wingfoil **latency** surface
//! ([`wingfoil::latency`]) — stamp wall-clock timestamps onto messages as
...
//! `wingfoil::latency` is unconditional in the engine. The module is
...
//! through `wingfoil::latency`'s [`record_stage_deltas`] /
//! [`format_latency_report`], the runtime-named entry points the typed
//! [`LatencyStats`](wingfoil::latency::LatencyStats) also delegates to. A
...
//!    `stream.stamp("a")`; here it is `wingfoil.stamp(stream, "a")` — the
...
//!    [`LatencyStats`](wingfoil::latency::LatencyStats) so the caller can
...
/// [`latency_stages!`](wingfoil::latency::latency_stages) generates for a
/// Rust caller.
...
/// [`Traced<T, L>`](wingfoil::latency::Traced).
///
...
impl DynLatencyStats {
    ...
    /// [`Runner::run`](wingfoil::interp::Runner::run) needs so a re-run
